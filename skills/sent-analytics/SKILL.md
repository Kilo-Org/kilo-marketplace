...
name: sent-analytics
description: >-
  Query Sent number capabilities and aggregate messaging, deliverability, and
  contact analytics with the Sent MCP tools. Use for dashboard metrics and
  date-bounded trends; use messaging-performance-analyzer for record-level
  delivery diagnosis.
metadata:
  category: data
  source:
    repository: 'https://github.com/sentdm/sent-plugin'
    path: skills/sent-analytics
