...
description: >-
  面向真实业务决策的多平台事实核验 Skill。正式支持深知晓、深知晓（深度研究）、豆包、腾讯元宝、DeepSeek 和通义千问，按用户输入动态选择 N≥1
  个平台。完整采集原回答、引用、截图与页面存证，拆解并对比关键事实，再按需使用可信搜索取得权威证据，交付四份独立报告和可追溯结论。基础采集与对比无需 API
  Key；可信搜索是权威核验阶段的可选增强。语义工作由 Codex、Claude Code、WorkBuddy 等当前运行载体完成，不调用外部大模型 API。
metadata:
  slug: fact-check-x
  ...
  source:
    ...
    ref: main
    commit: 4dd7eef0452a4c31e4b3b3b0d643c9daeea7fdbe
---
