...
name: orca-replay
description: >-
  Answers questions about a past agent run from its recording rather than from
  memory, and replays or forks that run. Use when asked why an earlier run did
  something, or to reproduce a failure.
license: Apache-2.0
compatibility: >-
  Requires the `orcareplay` npm package (Node 20+) with its MCP server
  registered as `orca`, and at least one recorded run in the project's
  .orca/runs directory.
requirements:
  mcps:
    - orcareplay
metadata:
  ...
  source:
    ...
    ref: main
    commit: 6ca75cadc6a2cd8d351077db35d9e70b2cac2f1a
---
