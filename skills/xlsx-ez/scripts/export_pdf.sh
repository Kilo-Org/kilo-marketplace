#!/usr/bin/env bash
# export_pdf.sh — Export a .xlsx file to PDF using LibreOffice headless.
#
# Usage: bash export_pdf.sh <input.xlsx> [output-dir]
#
# The output PDF is placed in output-dir (defaults to the same directory as
# input). The output filename matches the input stem: input.xlsx -> input.pdf.
#
# Install LibreOffice:
...
# ── Argument handling ────────────────────────────────────────────
if [[ $# -lt 1 ]]; then
  echo "Usage: $0 <input.xlsx> [output-dir]" >&2
  exit 1
fi
