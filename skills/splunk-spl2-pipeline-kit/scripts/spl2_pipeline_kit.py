...
GENERATED_MARKER = ".kilo-generated"
GENERATOR_ID = "splunk-spl2-pipeline-kit\n"
...
def parse_args(argv: Iterable[str] | None = None) -> argparse.Namespace:
    ...
    parser.add_argument(
        "--force",
        action="store_true",
        help="Replace an existing output directory only when it has this tool's generated marker.",
    )
...
def validate_output_dir(output_dir: Path) -> None:
    ...
    if output_dir.exists() and any((output_dir / name).exists() for name in PROJECT_MARKERS):
        raise ValueError(f"Refusing existing project-like output directory: {output_dir}")


def prepare_output_dir(output_dir: Path, *, replace: bool, force: bool) -> None:
    validate_output_dir(output_dir)
    marker = output_dir / GENERATED_MARKER
    if output_dir.exists():
        entries = list(output_dir.iterdir())
        if entries and (not marker.is_file() or marker.read_text(encoding="utf-8") != GENERATOR_ID):
            raise ValueError(
                f"Refusing to write to unowned output directory {output_dir}; "
                f"choose an empty directory or one generated by this tool."
            )
        if entries and replace:
            if not force:
                raise ValueError(f"Output directory already exists; pass --force to replace {output_dir}")
            shutil.rmtree(output_dir)
    output_dir.mkdir(parents=True, exist_ok=True)
    write_file(marker, GENERATOR_ID)
...
def main(args: argparse.Namespace | None = None) -> int:
    ...
    prepare_output_dir(
        output_dir,
        replace=args.phase in {"render", "all"},
        force=args.force,
    )
